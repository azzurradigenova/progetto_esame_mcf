...
#i dati seguono l'andamento di una distribuzione lognormale, per cui definisco una funzione opportuna per il fit
...
plt.figure()
plt.errorbar(bin_centers, hist, yerr=sigma,
             fmt='o', label="Monte Carlo", capsize=2)
plt.plot(x, lognormal_counts(x, N_fit, mu_fit, sigma_fit),
         'r-', label="Fit lognormale (curve_fit)")
plt.xlabel("Numero di hit")
plt.ylabel("Conteggi")
plt.legend()
plt.show()
...
plt.loglog(E_plot, power_law(E_plot, A_fit, alpha_fit),
           label=rf"Fit: $N = A E^{{\alpha}}$, $\alpha={alpha_fit:.2f}\pm{sigma_alpha:.2f}$")
...
cost = UnbinnedNLL(hits_pos, lognormal_pdf)
...
x = np.linspace(min(hits_pos), max(hits_pos), 300)
pdf_fit = lognormal_pdf(x, m.values["mu"], m.values["sigma"])
